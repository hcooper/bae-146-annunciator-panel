...
class MobiFlightVariableRequests:
    ...
    def __init__(self, simConnect, q):
    ...
    def _client_data_callback_handler(self, callback_data):
        ...
        if callback_data.dwDefineID in self.sim_vars:
            ...
            self.q.put(self.sim_vars[callback_data.dwDefineID])
            logging.debug("client_data_callback_handler: %s", self.sim_vars[callback_data.dwDefineID])  # on change
        ...
        # Response string of my_client
            ...
            response =  self._c_string_bytes_to_string(bytes(callback_data.dwData))
            logging.debug("client_data_callback_handler: get my_client response string: %s", response)  # list vars
        else:
